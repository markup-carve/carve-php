...
class IncludeExpanderTest extends TestCase
{
    ...
    public function testWarningsAreCappedButKeepOnePerRuleAndCountTheRest(): void
    {
        $converter = new CarveConverter();
        $source = implode("\n\n", array_fill(0, 40, '{{ missing.crv }}')) . "\n";
        $document = $converter->parse($source);
        $expander = new IncludeExpander($this->resolver([]), warningLimit: 5);

        $converter->transform($document, $expander);

        $this->assertCount(5, $expander->getWarnings());
        $this->assertSame(35, $expander->getSuppressedWarnings());
        $rules = array_unique(array_map(static fn ($w) => $w->getRule(), $expander->getWarnings()));
        $this->assertSame([IncludeExpander::RULE_UNRESOLVED], array_values($rules));
    }

    public function testACappedReportStillKeepsADistinctRule(): void
    {
        $converter = new CarveConverter();
        // One unresolved directive fills a cap of 1; the cycle that follows is
        // a rule not yet shown, so it survives rather than being counted away.
        $document = $converter->parse("{{ missing.crv }}\n\n{{ a.crv }}\n");
        $expander = new IncludeExpander($this->resolver(['a.crv' => '{{ a.crv }}']), warningLimit: 1);

        $converter->transform($document, $expander);

        $this->assertSame(
            [IncludeExpander::RULE_UNRESOLVED, IncludeExpander::RULE_CYCLE],
            array_map(static fn ($w) => $w->getRule(), $expander->getWarnings()),
        );
    }

    public function testAnIncludedNodeSaysWhichFileItsPositionIsMeasuredIn(): void
    {
        $converter = new CarveConverter();
        $converter->getParser()->enablePositionTracking();
        $document = $converter->parse("Parent one.\n\n{{ child.crv }}\n\nParent two.\n");
        $expander = new IncludeExpander(
            $this->resolver(['child.crv' => "Child A.\n\nChild B.\n"]),
            'main.crv',
        );
        ...
        $files = array_map(
            static fn ($node) => $node->getPos()?->file,
            array_values($expanded->getChildren()),
        );
        // Three paragraphs report line 1, 1 and 3; without the file they would
        // be indistinguishable from the parent's own coordinates.
        $this->assertSame([null, 'child.crv', 'child.crv', null], $files);
    }

    public function testANestedIncludeAttributesToTheGrandchild(): void
    {
        $converter = new CarveConverter();
        $converter->getParser()->enablePositionTracking();
        $document = $converter->parse("{{ child.crv }}\n");
        ...
        $this->assertSame(
            ['child.crv', 'deep.crv'],
            array_map(static fn ($node) => $node->getPos()?->file, array_values($expanded->getChildren())),
        );
    }

...
class IncludeDirectiveSerializationTest extends TestCase
{
    ...
    public function testTextThatIsNotAWellFormedDirectiveIsNotPreserved(string $source): void
    {
        $converter = CarveConverter::carve();

        $carve = $converter->render($converter->parse($source));

        // The writer is MINIMAL now, so a brace run that reads back as the same
        // text is emitted bare and its escaping proves nothing either way. What
        // matters is that a run which is not shape-well-formed is not treated
        // as a directive, which the round trip below is the evidence for.
        $this->assertSame($carve, $converter->render($converter->parse($carve)));
    }

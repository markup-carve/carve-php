...
- AST interchange preserves line-block ranges, explicit sections and block
  table cells. The Carve writer exposes a bounded conversion-diagnostics report
  for source structures and fields it cannot spell, including the CLI flag
  `--report-conversion-diagnostics` (#2308).
...
- **A code block the container closes reaches the content below its fence** (#2251). An item or a description body ends an unterminated fence by appending a closer the author never wrote, and the end of the block was resolved through that line, which belongs to no source line; the span collapsed onto the opening fence with the block's own content outside it. The end now falls on the last line the source holds, and the `list` and `list_item` above follow it.
- **An unresolved reference's given-back caption lines carry their positions** (#2250). A caption slot that resolves to nothing comes back to the paragraph as literal text, and neither that text nor the soft break before it was published with a `pos`, so the paragraph, and any list holding it, ended at the image. Both are verbatim source and are placed where carve-js and carve-rs place them.
- **A caption slot is settled through the finished document** (#2230). The slot held the container and the index it was seen at with a strong reference, and the walk re-parses subtrees, so over a 321 KB document 16 of 24 slots named a container nothing else referenced any more: settling wrote into a detached tree no reader sees, and the reference kept that tree alive to the end of the parse. The map is keyed weakly by the paragraph now and settling walks the finished document, which brings peak parse memory back to 30 MB from 42 MB. Output is unchanged.
- **A description body's fence opens on a closer written below a lazy line** (#2233). The body asked CARVE-P0-014's closer question of the lines it had collected, and that view stops at the line being classified, so a closer under a below-column line was invisible and the fence never armed; the line folded into a code block instead of ending the description. The search reads the source now, bounded by a new entry marker or a blank line no later line continues, and matches at the opener's authored column.
...
- **An empty `<ins>` or `<del>` is dropped without a report row** (#2113), like every other empty inline element the HTML importer drops.
- **The HTML importer keeps the space an inline element holding only whitespace rendered** (#2114): `<strong> </strong>` imports as `{* *}`, which is what the Carve writer writes for that tree, instead of dropping the element.

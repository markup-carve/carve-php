...
- **A line after a nested Markdown item's unclosed fence stays out of the item** (#2129). The importer writes a blank line after the closer when the Markdown had one or when the line leaves the list, so Carve no longer reads it as lazy content of the parent item.

...
- **The Markdown importer reduces an extended fence info string to its language** (#2128): ```` ```js title=x ```` writes ```` ```js ````, since Carve reads a longer info string as paragraph text. It applies at the top level, in a list item and in a quote.

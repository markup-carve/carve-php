...
- **A container takes PART 9 §17 L7's consumed `loose` boolean** (#1642, #1644, #1654, markup-carve/carve#1612, markup-carve/carve#1623). `{loose}` above a list or definition list renders the children as blocks and never reaches the HTML as an attribute. Both writers emit the key by the clause's decision procedure - write the body without it, read it back, keep it exactly where the looseness did not survive - so a one-item loose list, which has no "between items" for a blank line to stand in, keeps its `<p>`.
- **The table-of-contents nav carries an accessible name** (#1574, markup-carve/carve#1547, ruling markup-carve/carve#1509). Both TOC extensions write `aria-label` on the `<nav>` from a new `labels` key `tocNav`; an authored name wins, and the `<details>` shape is unchanged.
- **Table column metadata** (#1451, #1480, markup-carve/carve#1391). Alignment, vertical alignment and widths reach the AST as `table.columns` and `table_cell.valign`, with schema validation, ProseMirror fidelity, `<colgroup>` and CSS rendering, canonical writing, ListTable columns and footer rows, and lint coverage.
...
- **A Djot blank-line run does not split a list on import** (markup-carve/carve#1430, PART 9 §11 N1a). `carve migrate --from djot` cuts a run of three or more blank lines back to one where Carve alone would read it as a list boundary, so an imported list stays one list and an ordered list keeps its numbering. Lines are read through their block-quote prefix; runs of one or two blanks, runs inside a fenced block, runs before an item's own indented content and runs at another quote depth are byte-identical.
- **The HTML importer spells adjacent sibling lists with the hard boundary** (PART 9 §11 N1a). Two back-to-back lists import as the same marker separated by three blank lines instead of an alternating marker the importer never read from the source (#1290), which also makes it agree with carve-js and carve-rs. An explicit `data-marker`, and lists that already differ on a marker, numbering or task-list axis, keep the single blank line; the boundary holds at every nesting level, inside a block quote and for a run of three or more lists.
- **An imported nested container widens inward, as `carve fmt` writes it** (#1583, PART 9 §12). The importer's output is a fixed point of this engine's formatter again; a closer-shaped verse line is escaped rather than widened around.
- **The writer escapes per opener occurrence, not per unit** (#1533, markup-carve/carve#1533, PART 11 §2): `\{.note}` where the unit-scoped form wrote `\{\.note\}`. An opener RUN is still escaped whole. At spec pin `7cb4769` (#1519) canonical output changes on 20 corpus documents and the idle-escape ratchet drops from 25 entries / 59 escapes to 5 / 12, the same reading carve-js and carve-rs take on that pin.
- **The escalation search only offers the units the writer asks about** (#1577, markup-carve/carve#1546, PART 11 §2b), instead of rendering and re-parsing units no escape arm charges a character to.
- **A container's span ends at its last placed child** (markup-carve/carve#1522, markup-carve/carve#1524, markup-carve/carve#1530, PART 12 §4). A `list`, `list_item`, `block_quote` or `definition_list` no longer reaches over a hoisted definition, an unattached attribute block or the blank run after it. Published `pos` values move.
- **Every closerless container ends there, by one predicate** (#1638, PART 12 §4). `heading`, `footnote`, `definition_term` and `figure` derived their extent from the lines they consumed, so each ended one codepoint past its last child, taking in a trailing whitespace run or the line terminator. Published `pos` values move.
- **A css-mode panel is named, and `code-group` grows the same `mode`** (#1528, markup-carve/carve#1489, Extensions §13). Under `css` each panel carries `role="group"` named by its own tab's label; `aria` mode is unchanged. **BREAKING: an unknown `mode` on either extension now throws** instead of falling back to `css`, which stays the default.
...
- **The canonical writer separates two adjacent sibling lists with that boundary** (#1544) instead of indenting the later one by a space, which returned a list at a column the author never wrote.
- **HTML conversion has a conservative borrowed fast path, for configured converters too** (#1506, #1515). Simple blocks render from source slices without building the public AST, and the built-in extension stack compiles into typed borrowed events. `parse()`, alternate renderers, positions, warnings, strict/safe/profile modes, transformers, third-party extensions, non-ASCII input and documents over 64 KiB keep the owned AST, and nothing is published until the borrowed document re-renders byte-identically.
- **The default parse and extension dispatch paths do much less work** (#1489, #1490, #1491, #1498): one shared structural walk for the three collectors, early skips on documents with no opening bytes, delimiter-skipping inline text, no deep clones for idle Citations and Index, and a single pipe-table separator split. The quoted-comment closer search those prepasses hid is now linear.
- **The doubled run is the canonical arrow, in both families** (#1496, markup-carve/carve#1442). `<--` `-->` `<-->` and `<==` `==>` `<=>` convert. **BREAKING: `=>` no longer converts** - `key => value` was silently becoming an arrow in rendered output only. `<=` keeps `≤`.
...
- **The braced en dash is the node the bare run produces** (#1499): a positioned `smart_punctuation` node whose `value` keeps the authored `{--}`, not a text node holding the glyph, so `carve fmt` gives the four characters back.
...
- **A row is a row, in every table section** (markup-carve/carve#1459, PART 10 §7). `<thead>` and `<tfoot>` write one row per line, as `<tbody>` always did, in pipe tables and the list-table extension. Nothing renders differently.
- **A table cell's marker run ends at a space** (markup-carve/carve#1259, PART 9 §5 T11). Kind marker, alignment run and attribute block are one atomic run that must be followed by a space, so `|=hot= |` is a highlight, `|=a |` a data cell and `|{#x}=R|` literal text. A cell with no run is unchanged and formatted documents need no migration.
...
- **All definition and fence prepasses walk container prefixes by offset** (#1467, PART 9 §25). The remaining three scanners copied the rest of a line at every quote/list element, keeping the quadratic half of the prefix climb.
...
- **An alternating quote-and-bullet prefix is bounded before it can exhaust the stack** (#1456, PART 9 §25). Past the nesting cap it degrades to paragraph text, as carve-js and carve-rs already did.

### Fixed

- **An imported block-attribute line ends its own line** (#1653). Three of the ten callers of the importer's attribute-block writer added a second newline, so a list, a table and a definition list got a blank line between the attribute and the block it attaches to - where a block attribute floats forward and attaches to something else.
- **The HTML importer spells the looseness the layout cannot say** (#1648, PART 9 §17 L7). It writes source from the DOM rather than through `CarveRenderer`, so a one-item loose list - a document with a single footnote is exactly that - re-read tight and lost the `<p>` the imported tree recorded.
- **A dropped definition description is declared by what it writes, not what it holds** (#1649). `<dd><p> </p></dd>` and `<dd><ul></ul></dd>` hold elements and write nothing, so both were dropped with no `structure-unspellable` row.
- **A footnote body survives a blank run** (#1643, markup-carve/carve#1633). Two or more blank lines ended the definition and ejected the indented continuation to a top-level paragraph ahead of the endnotes section, moving the content backward past unrelated blocks. The three spellings of the rule in the block parser are now one predicate.
- **A position is spelled on the line its own offset is on** (#1639, PART 12 §4). `SourceMap::resolve()` advanced the column by the delta while leaving the segment's line alone, so a node could publish a line/column pair its own offset contradicts. The offset is the position and the pair is a spelling of it, so the offset wins.
- **A dropped empty description breaks the list rather than lending it** (#1646, markup-carve/carve#1636). An empty `<dd>` followed by another entry merged two terms into one run; the ceiling permits losing the description and no more.
- **An empty definition description declares the loss it already took** (#1615). The drop is permitted only where it is declared, so the `<dd>`'s path now reports `structure-unspellable`.
- **An import says what it drops, and a block that spells nothing keeps a list boundary** (#1633, PART 11 §7 and §10j). A block whose characters are all layout emitted no source and said nothing about it; it reports `element-dropped` now, while U+00A0, U+202F and U+3000 stay content. The canonical writer decides the hard boundary between two sibling lists by what a block SPELLS rather than by whether it is empty, so a paragraph holding one space no longer merges them.
- **The canonical writer honors the declared-loss ceiling too** (markup-carve/carve#1627). An ingested tree or `carve fmt` over one wrote a bare colon line for a description that spells nothing, which the parser reads back as a continuation of the term. This is where carve-js put its whole fix, so the two engines spend the ceiling in the same place.
- **A declared loss is a ceiling, not a license, and an endnotes section keeps its position** (#1629, markup-carve/carve#1627). An empty `<dd>` folded a colon into the term, split the list around a stray paragraph, or damaged the description above it; a description that writes nothing is decided by what it writes, so a non-breaking space keeps its line. An imported endnotes section with content after it re-rendered past that content.
- **An import keeps the meaning the HTML held, in three shapes** (#1615). A paragraph whose text opens `^ ` is escaped where it would otherwise make a figure of the block above it, decided by rendering both spellings rather than by enumerating hosts; a caret that opens a bracketed label is escaped in a span, an inline link and an abbreviation alike; and an empty `<ins>` or `<del>` reports `element-dropped` instead of vanishing silently.
- **A Markdown delimiter is owned per occurrence, not per character** (#1624). A closed backtick run is a code span this pass carries over and an unclosed one is ordinary text, which a character-keyed table cannot say: the unclosed run opened a Carve code span that ran to the end of the block. Every backtick of the run is escaped, not only the first.
- **A BBCode `noparse` body is literal at the block level too** (#1622). The escaper escaped what a line holds and nothing escaped what a line starts with, so a line-initial `- ` inside `[noparse]` reached the document as a list, and the blank run between two of them as the hard list boundary.
- **A fence keeps its blank runs behind whatever a container wrote to its left** (#1618). A code fence imported into a list item lost the blank lines in its payload, because the whole-document collapse armed its fence exemption off the trimmed line and an item's first block sits on the marker line. The unrecognized opener then left the flag unbalanced and cost the next top-level fence its blanks too.
- **A BBCode list keeps its structure, and every line keeps its block context** (#1619, #1620, #1621, #1623). A non-greedy pair of regexes closed an outer `[list]` on the inner `[/list]` and left the stray opener in the text; continuation lines landed at column 0, where a blank line ends the list; adjacent siblings were parted by alternating a marker the source never carried; and `[list=a]` matched no branch, so its `[*]` markers read as an emphasis span. The scan is depth-tracked with the same linear bound `parseQuotesWithDepth()` already used.
- **What sits between two lists is measured by what it writes** (#1617). An element that renders to nothing - an empty paragraph, `div`, `span`, `script` or `style`, or a run of them - erased the hard boundary and merged two imported lists into one; and a bare text run between them, stepped over as though it were not there, got a boundary it did not need.
- **The BBCode code fence trims where the body is still text** (#1612). `[code]` content is stashed behind a sentinel for the length of the pipeline, so the fence builder trimmed a key with no whitespace in it and every fenced block gained a blank line above and below its code.
- **A nested literal run no longer leaks its stash sentinel** (#1611). `BbcodeToCarve` restored the `[code]`, `[c]`, `[icode]` and `[noparse]` families in ONE pass, so `[noparse]` over a code run came back with the raw private-use key still in it. The restore loops now, bounded by the number of stashed spans, matching carve-js's `stashLiteralRuns` (markup-carve/carve-js#1375).
- **The no-destination rule outranks a provenance attribute, and unwrapped alt text is prose** (#1613, markup-carve/carve#1609). In `roundtrip` mode a `data-djot-*` attribute beside a blanked `href` rebuilt a link the rule says is not one; and an unwrapped `<img>`'s alt value reached the prose slot raw, so `alt="a *bold* b"` came back as emphasis the HTML never held.
- **A destination Carve cannot carry is not a destination** (#1605, markup-carve/carve#1609). An `<a>` with no `href`, an `<a href="">` and an `<img>` whose `src` is either imported as `[t]()` and `![t]()`, which are literal text. No link or image node is built now: the element's content stands in its place, carried by a span where an attribute survives, and reported as `element-unwrapped`.
- **The symbol sigil is escaped where it opens a symbol** (#1605, markup-carve/carve#1601, PART 11 §2). `<p>a :rocket: b</p>` imported as text that re-parses as a `symbol` node; it is now `a \:rocket: b`. A colon that closes no shortcode and a URL's colon are unchanged.
- **A table cell whose whole payload is a span marker is escaped** (#1605, markup-carve/carve#1601, PART 11 §6f). `<td>^</td>` imported as `| ^ |`, which re-reads as a rowspan marker and deleted the caret's own cell. `| \^ |` and `| \< |` are written now, while a real `rowspan` or `colspan` keeps the bare marker it needs.
- **Intraword emphasis survives HTML import** (#1602). `<p>Sy<strong>rup</strong>-free</p>` imported as `Sy*rup*-free`, which renders the asterisks as prose and throws the bold away; it is now `Sy{*rup*}-free`. All five single-character kinds pick bare or braced with `CarveRenderer::renderEmphasis`, so the importer's output is a `carve fmt` fixed point and agrees with carve-js.
- **`carve --help` names the HTML import modes the CLI accepts** (#1601). It advertised `safe (default) or raw`; `raw` is rejected by every engine, and `semantic` and `roundtrip` were unlisted.
- **A quoted fenced block keeps its blank lines on Djot import** (#1600). The blank-run collapse skips fenced blocks through a fence map that read the raw line, so a fence written inside a block quote was invisible to it and the pass rewrote the code block's own content. The map reads every line through its block-quote prefix now, matching `collapse_false_list_boundaries` in carve-rs.
- **The Markdown and plain-text renderers keep a figure caption's authored leading whitespace** (#1594, corpus 404). carve-js and carve-rs retained the authored tab; this engine removed it.
- **`carve fmt` keeps a heading's leading tab** (#1595, markup-carve/carve#1587, PART 11 §1). A heading's marker separator is a run of SPACES, so the first character that is not one begins the heading - the reader already kept it and the writer trimmed it away. Corpus 406.
- **An imported list item's attributes abut its marker** (#1587). They were written on an indented line below it, where a block attribute floats forward, so a one-block item lost the attribute and a degraded footnote's `id` went that way. `1.{#fn1} n` is what carve-js writes. A quoted marker attribute value may hold an escaped quote and a brace.
- **A non-`li` child of a list keeps its content and says so** (#1589, ruling markup-carve/carve-rs#1266). The HTML importer dropped it whole and in silence; it now goes through the ordinary block walk, is emitted ahead of the list, and reports `element-unwrapped`. A NESTED list's strays stay inside the item that holds the list, instead of splitting it apart at column zero.
...
- **A marker at an item's content column opens a sublist, first in the item or not** (markup-carve/carve#1517, PART 9 §24 C3). A later marker folded into the paragraph above it.
- **A blank verbatim line inside a block quote drops the prefix's trailing space** (markup-carve/carve#1544, PART 11 §7). The writer emitted `> ` inside a fenced block in a quote where an authored blank quote line is written `>`.
- **The AST-JSON ingest replaces U+0000, as the parse boundary already does** (#1562, markup-carve/carve#1528, PART 12 §21), so an ingested document renders like the same document written as source. It also closes a data loss where two abbreviation pairs differing only around a NUL shared the §10f key. The Markdown importer now replaces the character (CommonMark 2.3) rather than deleting it, and the BBCode importer rather than passing it on.
- **An import can take the `labels` map the HTML was rendered with** (markup-carve/carve#1500) via `new HtmlToCarve(labels: [...])`. A document rendered with `labels` carried names no English default recognized, so they were baked into the imported source. Passing nothing is unchanged.
- **A replaced NUL leaves codepoint offsets, and the text node keeps its position** (#1563, markup-carve/carve#1525, markup-carve/carve#1534, PART 0 INPUT, PART 12 §4): the substitution runs before the offset table is built. Published `pos` values move on documents holding a NUL; `srcByteLength` still counts the bytes that arrived.
- **The canonical writer spells two sibling sub-lists inside a list item** (#1553, markup-carve/carve#1501, PART 9 §11 N1a and §10i) at the item's content column, separated by the boundary, instead of behind the `+` marker at column 0. The same repair covers a sub-list below a blockquote in a tight item.
- **An escape escalation reaches the block that failed, not the document** (#1552, markup-carve/carve#1516, PART 11 §2b). The fallback reaches the smallest failing unit; every other candidate is written bare.
- **A block matched on a first-character fast path spans every line it consumed** (markup-carve/carve#1451, PART 12 §4). A `---json` opener and a `-` bullet list reported only their first line; 33 corpus documents now span as carve-js and carve-rs do.
- **A footnote definition inside a container ends with its body** (markup-carve/carve#1451, PART 12 §4), not one codepoint past it. At column 0 it still reaches the blank line, where its body may resume.
- **A tab control is `type="button"`, and two marked items select one tab** (#1537, markup-carve/carve#1504, Extensions §13.3 and §13.5). An `aria`-mode control inside a `<form>` submitted it; and the first `{selected}` mark now wins in both modes and both extensions.
- **Math survives HTML import** (#1543, #1554, markup-carve/carve#1518, ruling markup-carve/carve#1514, PART 9 §18, grammar §18). A `<span class="math inline">` / `<span class="math display">` imported as an attributed span holding raw `\(…\)` text; it now imports as the core math forms, keeping any authored id, class and `data-*`. A `<div class="math display">` imports as the core `$$` display form and never as the extension fence, so the equation is mathematics for a reader with no extension loaded. The MathML path no longer writes a trailing `$` / `$$`.
- **A block beside loose text in a container stays a block** (#1543). An imported container glued a bare text child onto its block sibling, so a tab panel became the literal `First::: tabs-panel` and the same seam swallowed a `<blockquote>`, `<ul>`, `<h2>`, `<pre>` and `<p>`.
- **An import drops an accessible name the renderer derives** (markup-carve/carve#1500). Re-importing one made it look authored and suppressed localization on the next render. Matched on value, over the untitled admonition, the endnotes section, a tab set and code group, a css-mode panel, a claimed diagram fence and an index back-link.
- **A static code-group panel label is a heading** (#1535, Extensions §2.5). The static flatten wrote an unindented `<p>` where `TabsExtension` writes an indented `<h3>`, keeping the label out of the document outline.
- **A tab or code-group label is text, not an attribute value** (#1538, PART 10 §2). The extensions escaped a double quote in the label element's text, where §2 escapes only `&`, `<` and `>`; the name in the attribute keeps its `&quot;`.
- **A titled admonition's generated title reference no longer survives import** (markup-carve/carve#1468). The import consumes the element the `aria-labelledby` pointed at, so the attribute was left naming nothing and suppressed the correct name on re-render. A reference pointing elsewhere, and an authored `aria-label`, still survive.
- **The index back-link says where it goes** (markup-carve/carve#1469). The k-th back-link is named `Back to {term} {k}` and shows `↩<sup>k</sup>`, mirroring PART 9 §16; the leading words come from the `indexBackref` label or a constructor argument.
- **A rendered diagram fence is an image with a name** (markup-carve/carve#1468). The hydration element and the static wrapper carry `role="img"` and a `label` defaulting to the fence's own word, written together or not at all. The no-renderer source fallback stays unnamed; an authored `role` or name wins.
- **A half-formed braced pair keeps its carets bare** (#1522, PART 11 §2). `{^x`, `x^}`, `{^`, `^}`, `{^}` and `{^{^x^}^}` were written with an invented `\^` that prevented nothing and split one text node into three. Where the pair completes inside one text run the opener still carries the escape.
- **A tab set and a code group say what they are** (#1526, markup-carve/carve#1468, Extensions §1.5). Both wrappers carry a `role` (`group`, or `tablist` in tabs ARIA mode) and a name, interactive and static alike, resolved as authored attribute, then the new `groupLabel` option, then `labels` under `tabsGroup` / `codeGroup`, then `Tabs` / `Code examples`. Both attributes are appended, so naming the set never moves an authored one.
- **The published tab CSS keeps the control reachable from the keyboard** (#1525). The docblock recipe hid the radios with `display: none`, taking them out of the focus order; they are clipped now and the focus ring is drawn on the label. `CodeGroupExtension` publishes the same recipe, where it previously published none.
...
- **A hyphen run that opens a word after whitespace is a flag, not a dash** (markup-carve/carve#1443, PART 9 §8). `git log --oneline` keeps its hyphens; `pages 1--10` and a trailing `text --` convert as before.
- **An empty braced pair keeps its carets bare** (#1516, markup-carve/carve#1482, PART 11 §4 with §1). `{^^}` holds no content, so escaping both carets split one text node into four. `}^p` and `[^` write bare too.
- **An empty comment writes its marker and nothing else** (#1514), instead of `%% ` with a trailing space no clause asks for.
- **A continuation marker attaches only a flush-left block** (#1501, markup-carve/carve#1436, §17). A line at any other column falls through to the ordinary column rules.
- **A lazy marker line's definition defines nothing** (#1486, #1487, #1488). The definition probe also re-registers legacy `addBlockPattern()` closures on itself, so a probe and the real parse cannot classify a document differently.
...
- **A table alignment marker run is validated as a whole** (#1472, #1474, #1478, markup-carve/carve#1344): duplicate axes such as `<<` fall back visibly, a run must open on a horizontal marker, and a vertical marker needs a horizontal partner or the `?` inherit, so `v>` and a lone `^` stay literal cell content. Also **an unmarked definition-shaped lazy continuation inside a quoted list is preserved** rather than consumed as a definition.

...
- **The HTML importer escapes a hyphen or dot run that would read back as a dash or an ellipsis** (#2101), with the Carve writer's bytes: `<p>a -- b ... c</p>` imports as `a \-\- b \.\.\. c`. A lone hyphen, a flag such as `--x` and two dots stay bare.

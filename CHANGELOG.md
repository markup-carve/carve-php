...
- **A Djot blank-line run does not split a list on import** (markup-carve/carve#1430, PART 9 §11 N1a). `carve migrate --from djot` cuts a run of three or more blank lines back to one where Carve alone would read it as a list boundary, so an imported list stays one list instead of two and an ordered list keeps its numbering. Lines are read through their block-quote prefix, so a list inside a quote is covered too. Runs of one or two blanks, runs inside a fenced block, runs before an item's own indented content and runs whose marker sits at another quote depth are byte-identical.
- **The HTML importer spells adjacent sibling lists with the hard boundary** (PART 9 §11 N1a). Two back-to-back `<ul>`s import as `-` / three blank lines / `-` instead of `-` / `*`, and two `<ol>`s as `1.` / three blank lines / `1.` instead of `1.` / `1)`. The importer no longer alternates the marker it never read from the source (#1290), which also makes it agree with carve-js and carve-rs. An explicit `data-marker` is still honored, lists that already differ on a marker, numbering or task-list axis keep the ordinary single blank line, and the boundary now holds at every nesting level, inside a block quote (where it is spelled with three `>` lines) and for a run of three or more lists.
...
- **A container's span ends at its last placed child** (markup-carve/carve#1522, markup-carve/carve#1524, PART 12 §4). A `list`, `list_item` or `block_quote` no longer reaches over a hoisted definition, an unattached attribute block or the blank run after it. Published `pos` values move.
- **A `definition_list`'s span ends at its last placed child too** (markup-carve/carve#1530, PART 12 §4). Published `pos` values move on those documents.
...
- **All definition and fence prepasses walk container prefixes by offset** (markup-carve/carve-php#1467, PART 9 §25). The remaining three scanners copied the rest of a line at every quote/list element, keeping the quadratic half of the prefix climb.
- **The heading-reference prescan reads a line's container prefix at an offset** (markup-carve/carve-php#1463, PART 9 §25). A 128 KB line of `> - ` repeated copied 4.1 GB.
- **An alternating quote-and-bullet prefix is bounded before it can exhaust the stack** (markup-carve/carve-php#1456, PART 9 §25). Past the nesting cap it degrades to paragraph text, as carve-js and carve-rs already did.

### Fixed

- **A nested literal run no longer leaks its stash sentinel** (#1611). `BbcodeToCarve` hides `[code]`, `[c]`, `[icode]` and `[noparse]` content behind a private-use key and restored the two families in ONE pass, so `[noparse]` over a code run - whose body holds a key spliced in moments earlier - came back with the raw U+E010 pair still in it: a sentinel escaping into user-visible text. The restore now loops, bounded by the number of stashed spans, matching carve-js's `stashLiteralRuns` (markup-carve/carve-js#1375). The reverse nesting and every unnested run are unchanged.
- **A destination Carve cannot carry is not a destination** (#1605, markup-carve/carve#1609). An `<a>` with no `href`, an `<a href="">` and an `<img>` whose `src` is either imported as `[t]()` and `![t]()`, which are literal text - four punctuation characters the HTML never held. No link or image node is built now: the element's content stands in its place, carried by a span where an attribute survives, and reported as `element-unwrapped`. The destination is never rebuilt - not from a `title`, not from the anchor's text, not from provenance in `roundtrip` mode - because PART 9 §25 blanks it deliberately.
- **The symbol sigil is escaped where it opens a symbol** (#1605, markup-carve/carve#1601, PART 11 §2). `<p>a :rocket: b</p>` imported as `a :rocket: b`, which re-parses as a `symbol` node, so under a configured symbol map the text stopped being the text the HTML held. It is now `a \:rocket: b`. A colon that closes no shortcode and a URL's colon are unchanged.
- **A table cell whose whole payload is a span marker is escaped** (#1605, markup-carve/carve#1601, PART 11 §6f). `<td>^</td>` imported as `| ^ |`, which re-reads as a rowspan marker: the cell above grew `rowspan="2"` and the caret's own cell was deleted outright. `| \^ |` and `| \< |` are written now, while a real `rowspan` or `colspan` keeps the bare marker it needs.
- **Intraword emphasis survives HTML import** (#1602). `<p>Sy<strong>rup</strong>-free</p>` imported as `Sy*rup*-free`, which renders the two asterisks as prose and throws the bold away; it is now `Sy{*rup*}-free`. All five single-character kinds - `*`, `/`, `_`, `~` and `=` - pick bare or braced with `CarveRenderer::renderEmphasis`, the rule the canonical writer uses, so the importer's output is a `carve fmt` fixed point and agrees with carve-js. `=` also stops over-bracing next to punctuation or a sibling construct: `(=b=) c`, not `({=b=}) c`.
- **`carve --help` names the HTML import modes the CLI accepts** (#1601). It advertised `safe (default) or raw`; `raw` is rejected by every engine, and `semantic` and `roundtrip` were unlisted. The help now names all three and marks `roundtrip` as trusted-input only.
- **A quoted fenced block keeps its blank lines on Djot import**. The blank-run collapse skips fenced blocks through a fence map that read the raw line, so a fence written inside a block quote was invisible to it and the pass rewrote lines that are the code block's own content. The map now reads every line through its block-quote prefix, matching `collapse_false_list_boundaries` in carve-rs.
- **`carve fmt` keeps a heading's leading tab** (#1595, markup-carve/carve#1587, PART 11 §1). A heading's marker separator is a run of SPACES, so the first character that is not one begins the heading - the reader already kept it, and the writer trimmed it away, so `##<SP><TAB>x` came back as `##<SP>x`. Corpus 406.
...
- **Math survives HTML import** (#1543, #1554, markup-carve/carve#1518, ruling markup-carve/carve#1514, PART 9 §18, grammar §18). A `<span class="math inline">` / `<span class="math display">` imported as an attributed span holding raw `\(…\)` text; it now imports as the core math forms, keeping any authored id, class and `data-*`. A `<div class="math display">` imports as the core `$$` display form and never as the extension ``` math ``` fence, so the equation is mathematics for a reader with no extension loaded. The `<math>` (MathML) path no longer writes a trailing `$` / `$$`.
...
- **A tab or code-group label is text, not an attribute value** (#1538, PART 10 §2). The extensions escaped a double quote in the label element's text, where §2 escapes only `&`, `<` and `>`; the name in the attribute keeps its `&quot;`. The spec's `46-tabs-css-panel-name` and `47-tabs-aria-panel-binding` now match byte for byte.
...
- **A hard break ends at column 1 of the following physical line** (markup-carve/carve-php#1457), including where the next line is a comment-only line the block layer removes.
- **A line block opened on a container's opener line is seen by both definition prepasses** (markup-carve/carve-php#1444).
- **A verse comment inside an image's ALT has one spelling**, a bare `%%` line, for inline and reference images alike (markup-carve/carve-php#1443).
- **An abutting attribute block attaches to a plus bullet** where the extension is enabled, as it does to every other marker (markup-carve/carve-php#1438).
- **The flatten separator and the content-column family follow their rulings** (PART 11 §1b, markup-carve/carve-php#1435): the separator is decided per token, an existing NBSP is kept, and table-cell and definition-part boundaries survive.
- **The caption slot reports the blocks it flattens**, against the DOM nodes it actually consumed (markup-carve/carve-php#1353).
- **A lazily collected or over-indented comment no longer closes a list item**, and no longer erases the fact that the block above it was invisible (markup-carve/carve-php#1458, follow-on to markup-carve/carve-php#1421).
- **A heading written at a container's content column leaves no paragraph open** (markup-carve/carve-php#1464).

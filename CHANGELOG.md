...
- **A Djot blank-line run does not split a list on import** (markup-carve/carve#1430, PART 9 §11 N1a). `carve migrate --from djot` cuts a run of three or more blank lines back to one where Carve alone would read it as a list boundary, so an imported list stays one list instead of two and an ordered list keeps its numbering. Runs of one or two blanks, runs inside a fenced block and runs before an item's own indented content are byte-identical.
